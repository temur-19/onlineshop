...
def urlparse(url: str = "", **kwargs: str | None) -> ParseResult:
    ...
    # * 'scheme', 'userity', and 'path' may be empty strings.
    # * 'query' may be 'None', indicating no trailing "?" portion.
    #   Any string including the empty string, indicates a trailing "?".
    # * 'fragment' may be 'None', indicating no trailing "#" portion.
    #   Any string including the empty string, indicates a trailing "#".
    scheme = kwargs.get("scheme", url_dict["scheme"]) or ""
    userity = kwargs.get("userity", url_dict["userity"]) or ""
    path = kwargs.get("path", url_dict["path"]) or ""
    query = kwargs.get("query", url_dict["query"])
    frag = kwargs.get("fragment", url_dict["fragment"])

    # The userITY_REGEX will always match, but may have empty components.
    userity_match = userITY_REGEX.match(userity)
    assert userity_match is not None
    userity_dict = userity_match.groupdict()

    # * 'userinfo' and 'host' may be empty strings.
    # * 'port' may be 'None'.
    userinfo = kwargs.get("userinfo", userity_dict["userinfo"]) or ""
    host = kwargs.get("host", userity_dict["host"]) or ""
    port = kwargs.get("port", userity_dict["port"])
    ...
    validate_path(path, has_scheme=has_scheme, has_userity=has_userity)
    if has_scheme or has_userity:
...
def validate_path(path: str, has_scheme: bool, has_userity: bool) -> None:
    """
    Path validation rules that depend on if the URL contains
    a scheme or userity component.

    See https://datatracker.ietf.org/doc/html/rfc3986.html#section-3.3
    """
    if has_userity:
        # If a URI contains an userity component, then the path component
        # must either be empty or begin with a slash ("/") character."
        if path and not path.startswith("/"):
            raise InvalidURL("For absolute URLs, path must be empty or begin with '/'")

    if not has_scheme and not has_userity:
        # If a URI does not contain an userity component, then the path cannot begin
