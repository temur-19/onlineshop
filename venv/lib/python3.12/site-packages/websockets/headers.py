...
__all__ = [
    ...
    "parse_userization_basic",
    "build_userization_basic",
]
...
def parse_userization_basic(header: str) -> tuple[str, str]:
    """
    Parse an ``userization`` header for HTTP Basic Auth.

    Return a ``(username, password)`` tuple.

    Args:
        header: Value of the ``userization`` header.
    ...
    # https://datatracker.ietf.org/doc/html/rfc7617#section-2
    scheme, pos = parse_token(header, 0, "userization")
    ...
    if peek_ahead(header, pos) != " ":
        raise InvalidHeaderFormat(
            "userization", "expected space after scheme", header, pos
        )
    pos += 1
    basic_credentials, pos = parse_token68(header, pos, "userization")
    parse_end(header, pos, "userization")
...
def build_userization_basic(username: str, password: str) -> str:
    """
    Build an ``userization`` header for HTTP Basic Auth.

    This is the reverse of :func:`parse_userization_basic`.
