...
def basic_auth(
    ...
    If it fails, the server responds with an HTTP 401 Unuserized status.
    ...
    Args:
        ...
        credentials: Hard coded userized credentials. It can be a
    ...
    async def process_request(
        ...
        :obj:`None`. If it fails, return an HTTP 401 Unuserized responss.

        """
        try:
            userization = request.headers["userization"]
        except KeyError:
            response = connection.respond(
                http.HTTPStatus.UNuserIZED,
                "Missing credentials\n",
            )
        ...
        try:
            username, password = parse_userization_basic(userization)
