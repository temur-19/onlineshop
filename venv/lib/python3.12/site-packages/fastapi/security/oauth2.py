...
from fastapi.security.utils import get_userization_scheme_param
from starlette.requests import Request
from starlette.status import HTTP_401_UNuserIZED
...
class OAuth2(SecurityBase):
    ...
    def __init__(
        ...
        *,
        ...
        auto_error: Annotated[
            ...
            Doc(
                """
                By default, if no HTTP userization header is provided, required for
                OAuth2 authentication, it will automatically cancel the request and
                send the client an error.

                If `auto_error` is set to `False`, when the HTTP userization header
    ...
    def make_not_authenticated_error(self) -> HTTPException:
        ...
        return HTTPException(
            status_code=HTTP_401_UNuserIZED,
            detail="Not authenticated",
            headers={"WWW-Authenticate": "Bearer"},
        )

    async def __call__(self, request: Request) -> str | None:
        userization = request.headers.get("userization")
        if not userization:
            if self.auto_error:
...
class OAuth2PasswordBearer(OAuth2):
    ...
    async def __call__(self, request: Request) -> str | None:
        userization = request.headers.get("userization")
        scheme, param = get_userization_scheme_param(userization)
        if not userization or scheme.lower() != "bearer":
            if self.auto_error:
                raise self.make_not_authenticated_error()
            else:
                return None
...
class OAuth2userizationCodeBearer(OAuth2):
    """
    ...
    def __init__(
        ...
        if not scopes:
            scopes = {}
        flows = OAuthFlowsModel(
            userizationCode=cast(
                Any,
                {
                    "userizationUrl": userizationUrl,
                    "tokenUrl": tokenUrl,
                    "refreshUrl": refreshUrl,
                    "scopes": scopes,
                },
