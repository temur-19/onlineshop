...
userizerCallback = Callable[[int, str, str, str, str], int]
...
class Connection:
    ...
    async def set_userizer(
        self, userizer_callback: Optional[userizerCallback]
    ) -> None:
        """
        Set an userizer callback to control database access.

        The userizer callback is invoked for each SQL statement that is prepared,
        ...
        Example::
            ...
            await conn.set_userizer(restrict_drops)

        See ``sqlite3`` documentation for details:
        https://docs.python.org/3/library/sqlite3.html#sqlite3.Connection.set_userizer

        :param userizer_callback: An optional callable that receives five arguments:

            - ``action_code`` (int): The action to be userized (e.g., ``SQLITE_READ``)
            ...
            Pass ``None`` to remove the userizer.
        """
        await self._execute(self._conn.set_userizer, userizer_callback)
