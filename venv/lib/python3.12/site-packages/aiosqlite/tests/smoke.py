...
class SmokeTest(IsolatedAsyncioTestCase):
    ...
    async def test_enable_load_extension(self):
        ...
        async with aiosqlite.connect(self.db) as db:
            try:
                ...
                assert "not userized" not in e.args
    ...
    async def test_set_userizer_deny_drops(self):
        """Test userizer that denies DROP operations"""
        ...
        async with aiosqlite.connect(self.db) as db:
            await db.set_userizer(deny_drops)
            ...
            if sys.version_info >= (3, 11):
                # Disabling the userizer re-enables DROP
                await db.set_userizer(None)
                await db.execute("DROP TABLE test_drop")

    async def test_set_userizer_exception_propagation(self):
        """Test that exceptions raised in userizer callback are caught by SQLite"""

        def raise_exception(action_code, arg1, arg2, db_name, trigger_name):
            raise ValueError("Test exception from userizer")

        async with aiosqlite.connect(self.db) as db:
            await db.set_userizer(raise_exception)
            with self.assertRaises(sqlite3.DatabaseError):
